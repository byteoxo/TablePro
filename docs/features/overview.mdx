...
TablePro opens with a welcome window: an actions panel on the left (Create Connection, Import from App, Try Sample Database) and your saved connections on the right. Search sits in the connections panel header; press `Cmd+F` to focus it. The sample is a bundled Chinook SQLite database. Reset it any time from **File > Reset Sample Database**.
...
<CardGroup cols={2}>
  ...
  <Card title="Cell & Row Viewers" icon="brackets-curly" href="/features/json-viewer">
