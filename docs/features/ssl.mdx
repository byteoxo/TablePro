...
<Frame caption="SSL tab in the connection form">
...
New connections pick the mode that matches each driver's native behavior. Open the SSL tab on any connection to see the driver-specific guidance.
...
| Snowflake, BigQuery, DynamoDB, Cloudflare D1, libSQL / Turso | Always encrypted | HTTPS-based drivers manage TLS themselves; no SSL tab |
