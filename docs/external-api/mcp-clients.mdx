...
- **`404 Session not found`** (JSON-RPC code `-32001`): the session hit the 15-minute idle timeout or the server restarted. Drop the cached `Mcp-Session-Id` and start a new `initialize` handshake; most clients do this automatically.
...
**Port conflict.** If the configured port (default 23508) is taken, TablePro falls back to a free port in the 51000-52000 range and rewrites the handshake file. The stdio bridge re-reads it automatically.
