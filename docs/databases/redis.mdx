...
TablePro connects to standalone Redis servers with hiredis. Keys are grouped by colon-separated namespaces in the sidebar. Values display with type-aware formatting in the data grid (strings, hashes, lists, sets, sorted sets, JSON).
...
Every command passes through to the server as-is, including ones TablePro does not recognize; only the result formatting is type-aware.
