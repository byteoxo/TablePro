...
The plugin uses either `rledger` (the rustledger project) or Python Beancount to parse ledgers. Install one of those backends separately. When both are available, TablePro uses `rledger`; Python Beancount is used as a fallback for browsing projected SQL tables.
...
<Steps>
  <Step title="Create a new connection">
    Open TablePro and click **New Connection** or press **Cmd+N**.
  </Step>
